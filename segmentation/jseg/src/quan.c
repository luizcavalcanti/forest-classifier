...
{  
  int i,j,l,mini,minj,total,k,ei,endloop;
  float mindist,avgJC,oldJC,newthresh,maxJ,olddist,thresh2,*cent,ST,*A;
int debug=0;
...
{
  int iy,ix,jy,jx,npeer,window,j,k,winarea,*index,*index2;
  float *peer,*dif,*dif2,avg=0.0,*D,D1,D2,**A1,mean1,mean2;
  int J1,J2,nnoise,J;
...
{
  int iy,ix,ny2,nx2,imgsize,imgsize2,jy,jx,nnoise,i,j,k,l,l1,sy,sx,ey,ex;
  int window,winarea,count,*count1,*index;
  unsigned char *cmap0;
