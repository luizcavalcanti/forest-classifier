...
int      numcols;                /* GIF parameter: # of colors */
float    normaspect;             /* GIF parameter: pixel aspect ratio */
...
/*****************************/
int LoadGIF(fname, pinfo, imagesize)
     char *fname;
     PICINFO *pinfo;
     int	*imagesize;
/*****************************/
{
  ...
  if (fread(dataptr, (size_t) 1, (size_t) filesize, fp) != filesize) 
  ...
  while (1) {
	...
	if (DEBUG) fprintf(stderr,"Graphic Control extension\n\n");
        if (DEBUG) fprintf(stderr, "%s:  %s", bname, 
	...
	if (DEBUG) fprintf(stderr,"%s:  Unknown extension 0x%02x in GIF file.  Ignored.\n",
		bname, fn);
